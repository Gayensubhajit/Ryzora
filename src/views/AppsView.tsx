    ...
    >
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-24 space-y-3">
          ...
          <p className="text-xs text-[var(--rz-text-muted)] font-medium">Loading applications catalog...</p>
        </div>
      ) : filteredPackages.length === 0 ? (
        <div className="text-center py-24 space-y-3">
          <Layers size={36} className="mx-auto text-[var(--rz-text-faint)]" />
        ...
        <div className="space-y-10">
          ...
          {!searchQuery && selectedCategory === "All" && featuredHeroApp && (
            <div
              ...
              className="relative w-full rounded-2xl md:rounded-3xl p-8 flex flex-col md:flex-row items-center justify-between gap-8 bg-[var(--rz-surface)] border border-[var(--rz-border)] shadow-md hover:shadow-lg hover:border-[var(--rz-border-strong)] transition-all duration-300 cursor-pointer group overflow-hidden"
            >
              {/* Very subtle atmospheric radial glow behind artwork */}
              <div
                className="absolute right-0 top-0 w-96 h-96 rounded-full blur-3xl pointer-events-none opacity-10 dark:opacity-15"
                style={{ background: "radial-gradient(circle, #3B82F6 20%, #6366F1 50%, transparent 70%)" }}
              />

              <div className="space-y-3 z-10 max-w-xl">
                <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold tracking-wider uppercase bg-blue-600/10 text-blue-600 dark:text-blue-400 border border-blue-500/20">
                  Featured application
                </span>
                <h2 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-[var(--rz-text)] group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                  Firefox
                </h2>
                <div className="text-xs font-semibold text-[var(--rz-text-muted)] flex items-center gap-1.5">
                  <span>Mozilla</span>
                  <CheckCircle2 size={12} className="text-blue-600 dark:text-blue-400" />
                </div>
                <p className="text-sm text-[var(--rz-text-secondary)] leading-relaxed max-w-lg">
              ...
              <div className="shrink-0 p-3 rounded-3xl bg-[var(--rz-surface-elevated)] border border-[var(--rz-border)] group-hover:scale-105 transition-transform duration-300 shadow-xs">
                ...
                {popularPicks.map((app) => {
                  ...
                  return (
                    <div
                      ...
                      className="group flex flex-col items-center text-center p-4 rounded-2xl bg-[var(--rz-surface)] hover:bg-[var(--rz-surface-hover)] border border-[var(--rz-border-subtle)] hover:border-[var(--rz-border-strong)] transition-all duration-150 ease-out cursor-pointer shadow-xs hover:shadow-sm"
                    >
                      <div className="p-1 mb-3 rounded-2xl transition-transform group-hover:scale-105 duration-200">
                        <AppIcon appId={app.id} size="lg" />
                      </div>
                      <span className="text-xs font-bold text-[var(--rz-text)] group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors truncate w-full">
                        {meta.displayName}
                      </span>
                      ...
                      {isInstalled && (
                        <span className="mt-2 text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border border-emerald-500/25 font-semibold">
                          Installed
                        </span>
                      )}
                    </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b border-[var(--rz-border-subtle)] pb-2">
              <h2 className="text-lg font-bold tracking-tight text-[var(--rz-text)]">
                {searchQuery
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
                {[
                  ...
                  <button
                    ...
                    className="flex items-center gap-3 p-3.5 rounded-xl bg-[var(--rz-surface)] hover:bg-[var(--rz-surface-hover)] border border-[var(--rz-border-subtle)] hover:border-[var(--rz-border-strong)] text-left transition-all cursor-pointer group shadow-xs hover:shadow-sm"
                  >
