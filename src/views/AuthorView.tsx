...
export const AuthorView: React.FC = () => {
  ...
  // Stage C: Store Distribution (Phase 11)
  ...
  const [distributionOutputDir, setDistributionOutputDir] = useState("~/RyzoraReleases");
  const [isAuditing, setIsAuditing] = useState(false);
  const [storeAuditReport, setStoreAuditReport] = useState<StoreAuditReport | null>(null);
  ...
  const handleRunStoreAudit = async () => {
    if (!authoringResult?.package_dir) {
      setToast({
        message: "Please build the package bundle (Phase A) before running store audit",
        type: "warning",
      });
      return;
    }
    setIsAuditing(true);
    try {
      const res = await auditStoreSubmission(authoringResult.package_dir);
      setStoreAuditReport(res);
      if (res.passed) {
        setToast({
          message: `Pre-flight store audit passed with score ${res.score}/100!`,
          type: "success",
        });
      } else {
        setToast({
          message: `Store audit identified issues (score ${res.score}/100)`,
          type: "warning",
        });
      }
    } catch (e: any) {
      setToast({
        message: `Store audit failed: ${e?.message || e}`,
        type: "warning",
      });
    } finally {
      setIsAuditing(false);
  ...
  const handleBuildDistribution = async () => {
    if (!authoringResult?.package_dir) {
      setToast({
        message: "Please build the package bundle first",
        type: "warning",
      });
      return;
    }
    setIsBuildingDistribution(true);
    try {
      const res = await buildDistributionRelease(
        authoringResult.package_dir,
        distributionOutputDir,
        releaseChannel,
        releaseNotes || undefined,
        maintainerHandle || undefined
      );
      setDistributionResult(res);
      setToast({
        message: `Distribution bundle generated for ${res.channel} channel!`,
        type: "success",
      });
    } catch (e: any) {
      setToast({
        message: `Distribution release failed: ${e?.message || e}`,
        type: "warning",
      });
    } finally {
      setIsBuildingDistribution(false);
    }
  };

  const handleCopySubmission = () => {
    if (!distributionResult?.submission_text) return;
    navigator.clipboard.writeText(distributionResult.submission_text);
    setCopiedSubmission(true);
    setToast({
      message: "GitHub PR submission template copied to clipboard!",
      type: "success",
    });
    ...
    <div className="space-y-6 pb-12 animate-fade-in">
      ...
      {currentStep === 5 && (
        <div className="bg-[var(--bg-card)] border border-[var(--border-subtle)] rounded-2xl p-6 shadow-sm space-y-6">
          ...
          {/* Stage C: Store Distribution & Community Submission (Phase 11) */}
          <div className="p-5 rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] space-y-5">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm font-bold text-[var(--text-primary)] flex items-center gap-2">
                  <Sparkles className="w-4 h-4 text-purple-400" />
                  Phase C: Store Distribution & Community Release Bundle
                </h3>
                <p className="text-xs text-[var(--text-muted)] mt-0.5">
                  Audit against zero-binary rules, assign release channel, and generate deterministic release artifacts (`release.json`, `checksums.sha256`, `SUBMISSION.md`).
                </p>
              </div>
              <span className="px-2.5 py-1 rounded-full text-[10px] font-semibold bg-purple-500/10 text-purple-300 border border-purple-500/30">
                Phase 11 Store Ready
              </span>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Left Column: Channel, Maintainer, Notes */}
              <div className="space-y-4">
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <button
                      ...
                      className={`px-3 py-2 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 border transition-all ${
                        releaseChannel === "stable"
                          ...
                          : "bg-[var(--bg-card)] text-[var(--text-muted)] border-[var(--border-subtle)] hover:bg-[var(--bg-card-hover)]"
                      }`}
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => setReleaseChannel("nightly")}
                      className={`px-3 py-2 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 border transition-all ${
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={distributionOutputDir}
                    onChange={(e) => setDistributionOutputDir(e.target.value)}
                    placeholder="~/RyzoraReleases"
                    className="w-full px-3 py-1.5 rounded-lg bg-[var(--bg-card)] border border-[var(--border-subtle)] text-xs font-mono focus:outline-none focus:border-[var(--accent)]"
                  />
                </div>
              ...
              {/* Right Column: Pre-Flight Audit & Generate Bundle */}
              <div className="space-y-4 flex flex-col justify-between">
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-semibold text-[var(--text-muted)]">
                      Store Pre-Flight Validation
                    </span>
                    <button
                      type="button"
                      onClick={handleRunStoreAudit}
                      disabled={isAuditing || !authoringResult}
                      className="px-3 py-1 rounded-lg bg-[var(--bg-card)] border border-[var(--border-subtle)] text-[11px] font-medium hover:border-[var(--accent)] text-[var(--text-primary)] disabled:opacity-50 flex items-center gap-1.5"
                    >
                      {isAuditing ? (
                        <>
                      ...
                      <div className="flex items-center justify-between">
                        <span className="font-bold flex items-center gap-1.5">
                          {storeAuditReport.passed ? (
                            ...
                            <>
                              <AlertTriangle className="w-4 h-4 text-amber-400" />
                              Store Audit Issues Found
                            </>
                          )}
                        </span>
                        <span className="font-mono px-2 py-0.5 rounded bg-black/30 font-semibold text-[11px]">
                          Score: {storeAuditReport.score}/100
                        </span>
                      </div>

                      <div className="space-y-1 text-[11px] text-[var(--text-muted)] font-mono">
                        <div className="flex items-center justify-between">
                          <span>Zero Compiled Binaries:</span>
                          <span className={storeAuditReport.binary_executables_found.length === 0 ? "text-emerald-400" : "text-rose-400"}>
                            {storeAuditReport.binary_executables_found.length === 0 ? "Verified (0)" : `${storeAuditReport.binary_executables_found.length} detected`}
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span>Zero Script Execution Hooks:</span>
                          <span className={storeAuditReport.script_hooks_found.length === 0 ? "text-emerald-400" : "text-rose-400"}>
                            {storeAuditReport.script_hooks_found.length === 0 ? "Verified (0)" : `${storeAuditReport.script_hooks_found.length} detected`}
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span>Tree Hash:</span>
                          <span className="text-[var(--text-primary)]">
                            {storeAuditReport.tree_hash ? `${storeAuditReport.tree_hash.slice(0, 16)}...` : "None"}
                          </span>
                        </div>
                      </div>

                      {storeAuditReport.warnings.length > 0 && (
                        <div className="pt-1 border-t border-white/10 text-[10px] text-amber-200">
                          {storeAuditReport.warnings[0]}
                        </div>
                      )}
                    </div>
                  ) : (
                    <div className="p-3.5 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-card)] text-xs text-[var(--text-muted)] flex items-center gap-2">
                      <ShieldCheck className="w-4 h-4 text-[var(--text-faint)]" />
                      Run pre-flight validation to verify zero compiled binaries and declarative safety before building release bundle.
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-3">
                  <button
                    type="button"
                    onClick={handleBuildDistribution}
                    disabled={isBuildingDistribution || !authoringResult}
                    className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-medium text-xs hover:opacity-90 disabled:opacity-50 shadow-md shadow-purple-900/20"
                  >
                    {isBuildingDistribution ? (
                      <>
                        <RefreshCw className="w-4 h-4 animate-spin" />
                        Generating Release Bundle & Artifacts...
                      </>
                    ) : (
                      <>
                        <Sparkles className="w-4 h-4" />
                        Build Distribution Release Bundle
                      </>
                    )}
                  </button>

                  {distributionResult && (
                    <div className="p-3.5 rounded-xl bg-purple-500/10 border border-purple-500/30 text-purple-200 text-xs space-y-2.5">
                      <div className="font-semibold flex items-center justify-between">
                        <span className="flex items-center gap-1.5 text-purple-300">
                          <Check className="w-4 h-4 text-purple-400" />
                          Release Bundle Ready!
                        </span>
                        <span className="uppercase text-[10px] font-mono px-1.5 py-0.5 rounded bg-purple-500/20 text-purple-300">
                          {distributionResult.channel}
                        </span>
                      </div>

                      <div className="text-[11px] font-mono space-y-0.5 text-purple-300/80">
                        <div>Dir: {distributionResult.bundle_dir}</div>
                        <div>Tree Hash: {distributionResult.tree_hash.slice(0, 16)}...</div>
                        <div>Generated: `release.json`, `checksums.sha256`, `SUBMISSION.md`</div>
                      </div>
                      ...
                      <div className="flex items-center gap-2 pt-1">
                        <button
                          ...
                          className="flex-1 flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg bg-purple-600 hover:bg-purple-500 text-white font-medium text-xs shadow-sm"
                        >
                          {copiedSubmission ? (
                            <>
                        ...
                        <a
                          href="https://github.com/Gayensubhajit/Ryzora/pulls"
                          target="_blank"
                          rel="noreferrer"
                          className="px-3 py-1.5 rounded-lg bg-[var(--bg-card)] border border-[var(--border-subtle)] text-xs text-[var(--text-primary)] hover:border-[var(--accent)] font-medium"
                        >
                          GitHub PRs
                        </a>
                      </div>
