...
import { CheckCircle2, AlertTriangle, ShieldAlert, Monitor, Cpu, Layers, ShieldCheck, Loader2, ChevronDown, ChevronUp } from "lucide-react";
...
export const CompatibilityPanel: React.FC<CompatibilityPanelProps> = ({
  ...
  const renderSystemIntegrationCard = () => {
    ...
    return (
      <div className="p-4 rounded-xl border border-[var(--rz-border-subtle)] bg-[var(--rz-surface)] text-[var(--rz-text)] shadow-sm my-3">
        {/* Header line */}
        <div className="flex items-center justify-between pb-3 border-b border-[var(--rz-border-subtle)]">
          ...
          <span className="px-2 py-0.5 rounded text-[10px] font-mono font-semibold bg-[var(--rz-surface-elevated)] border border-[var(--rz-border-subtle)] text-[var(--rz-text-muted)]">
            {systemReport.desktop} · {systemReport.display_server}
          </span>
        </div>

        {/* 4 Compact Badges: HYPRLAND / HYPRLOCK / HYPRIDLE / SDDM */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 pt-3 text-[11px]">
          <div className="p-2.5 rounded-lg bg-[var(--rz-surface-elevated)] border border-[var(--rz-border-subtle)] text-center sm:text-left">
            <div className="text-[10px] font-mono font-bold text-[var(--rz-text-muted)] uppercase tracking-wider">Hyprland</div>
            <div className="font-semibold text-[var(--rz-text)] mt-0.5 flex items-center justify-center sm:justify-start gap-1">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
              <span>{systemReport.display_server || "Wayland"}</span>
            </div>
          </div>

          <div className="p-2.5 rounded-lg bg-[var(--rz-surface-elevated)] border border-[var(--rz-border-subtle)] text-center sm:text-left">
            <div className="text-[10px] font-mono font-bold text-[var(--rz-text-muted)] uppercase tracking-wider">Hyprlock</div>
            <div className="font-semibold text-[var(--rz-text)] mt-0.5 flex items-center justify-center sm:justify-start gap-1">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
              <span>Available</span>
            </div>
          ...
          <div className="p-2.5 rounded-lg bg-[var(--rz-surface-elevated)] border border-[var(--rz-border-subtle)] text-center sm:text-left">
            <div className="text-[10px] font-mono font-bold text-[var(--rz-text-muted)] uppercase tracking-wider">Hypridle</div>
            <div className="font-semibold text-[var(--rz-text)] mt-0.5 flex items-center justify-center sm:justify-start gap-1">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
              <span>{systemReport.idle_config ? "Configured" : "Active"}</span>
            </div>
          </div>

          <div className="p-2.5 rounded-lg bg-[var(--rz-surface-elevated)] border border-[var(--rz-border-subtle)] text-center sm:text-left">
            <div className="text-[10px] font-mono font-bold text-[var(--rz-text-muted)] uppercase tracking-wider">SDDM</div>
            <div className="font-semibold text-[var(--rz-text)] mt-0.5 flex items-center justify-center sm:justify-start gap-1">
              <span className={`w-1.5 h-1.5 rounded-full ${isGdmActive ? "bg-amber-400" : "bg-emerald-400"}`} />
              <span>{isGdmActive ? "Unsupported" : "Ready"}</span>
            </div>
          </div>
        </div>

        {/* Expandable Disclosure Toggle */}
        <div className="pt-2 mt-2 border-t border-[var(--rz-border-subtle)]">
          <button
            type="button"
            onClick={() => setShowEvidence(!showEvidence)}
            className="flex items-center justify-center gap-1.5 w-full text-[11px] font-medium text-[var(--rz-text-muted)] hover:text-[var(--rz-text)] transition-colors py-1 cursor-pointer"
          >
            <span>{showEvidence ? "Hide detection details" : "View detection details"}</span>
            {showEvidence ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
          </button>

          {showEvidence && (
            <div className="mt-2.5 space-y-2 text-[11px] animate-in fade-in duration-150">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 p-2.5 rounded-lg bg-[var(--rz-surface-elevated)] border border-[var(--rz-border-subtle)] text-[11px]">
                <div>
                  <span className="text-[10px] font-mono uppercase text-[var(--rz-text-muted)] block">Session Lock Provider</span>
                  <span className="font-bold text-[var(--rz-text)]">{systemReport.session_lock_provider}</span>
                </div>
                <div>
                  <span className="text-[10px] font-mono uppercase text-[var(--rz-text-muted)] block">Display Manager</span>
                  <span className="font-bold text-[var(--rz-text)]">{systemReport.login_manager}</span>
                </div>
                <div>
                  <span className="text-[10px] font-mono uppercase text-[var(--rz-text-muted)] block">Idle Trigger Provider</span>
                  <span className="font-bold text-[var(--rz-text)]">{systemReport.idle_provider}</span>
                </div>
              </div>
              ...
              <div className="p-2.5 rounded-lg bg-[var(--rz-bg)] font-mono text-[10px] text-[var(--rz-text-muted)] space-y-1 max-h-40 overflow-y-auto border border-[var(--rz-border-subtle)]">
                <div className="text-[10px] font-bold text-[var(--rz-text-secondary)] mb-1 uppercase font-sans">Diagnostic Signals:</div>
    ...
    <div className="p-3.5 rounded-xl border border-emerald-500/30 bg-emerald-500/10 text-emerald-900 dark:text-emerald-200 my-3 text-xs flex items-start gap-2.5">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-center gap-3 mt-2 text-[11px] text-emerald-800 dark:text-emerald-300/80">
          ...
          <span>·</span>
          <span className="flex items-center gap-1 font-semibold text-emerald-400"><ShieldCheck className="w-3 h-3" /> Safe to install · Reversible</span>
        </div>
      </div>
    </div>
    );
